...
func (s *SSE) handleSubscribe(ctx *fasthttp.RequestCtx, ip string, authorized bool) {
	...
	ctx.SetBodyStreamWriter(func(sw *bufio.Writer) {
		...
		for {
			...
			if idx == len(eventCases) { // event from ping channel
				...
				if err := sendEvent(sw, heartbeat); err != nil {
					// stop listen
					return
				}
				lastMessageAt = now
			...
			err = clients[idx].ExecuteAll(lastEventId, func(e *Event) error {
				delivered++
				return sendEvent(sw, []byte("event: message"+
...
func sendEvent(w *bufio.Writer, data []byte) error {
